...
export const mapUserProfile = (input: unknown): User | null => {
  ...
  // Admins must never be treated as 'visitante' — default to 'analista'
  const defaultQualification = role === 'admin' ? 'analista' : 'visitante';
  ...
  if (profile.qualification !== null && profile.qualification !== undefined) {
    ...
    if (validQualifications.includes(qualValue as (typeof validQualifications)[number])) {
      // For admins, override 'visitante' to 'analista' even if explicitly stored
      qualification = (role === 'admin' && qualValue === 'visitante')
        ? 'analista'
        : qualValue as 'visitante' | 'cliente' | 'analista';
    } else {
      log.warn('Invalid qualification value, defaulting');
