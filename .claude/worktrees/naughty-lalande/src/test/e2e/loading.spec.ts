...
test.describe('Carregamento de Recursos', () => {
  ...
  test('deve carregar o arquivo CSS principal', async ({ page }) => {
    const cssLoaded = page
      .waitForResponse(response => response.url().includes('index.css') || response.url().includes('.css'), {
        timeout: 5000,
      })
      .catch(() => null);
  ...
  test('deve carregar o React sem erros no console', async ({ page }) => {
    ...
    const criticalErrors = consoleErrors.filter(
      error => !error.includes('Warning') && !error.includes('should not be used in production'),
    );
