...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const canAccessRh =
    (user?.role === 'admin' || user?.role === 'client' || user?.qualification === 'analista');
          ...
          <nav className="space-y-0.5 px-2">
            ...
            {agents
              ...
              .map(agent => {
                const isActive = activeAgentId === agent.id;
                const isLocked = agent.status !== 'active';
                const isAllowedForVisitor = true; // Visitors can now access all agents
                const isLockedForVisitor = false;
        ...
        <div className="p-3 border-t border-ai-border bg-ai-bg shrink-0">
          ...
          {onSwitchToInttegra && user?.role === 'admin' && (
            <button
              ...
              className="w-full flex items-center gap-2 px-3 py-2 mb-3 rounded-md text-sm font-medium transition-colors text-ai-subtext hover:text-emerald-600 hover:bg-emerald-50"
          ...
          {user && (
            <div className="mb-3 px-2 flex items-center gap-3">
              ...
              <div className="flex-1 overflow-hidden">
                ...
                <p className="text-[9px] text-ai-subtext font-bold uppercase tracking-wide mt-1">v{APP_VERSION} SaaS</p>
              </div>
            </div>
          )}
