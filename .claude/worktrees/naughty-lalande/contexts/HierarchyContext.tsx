...
// VISITOR_ANALYST_ID is the admin user (antonio@inttegra.com) who owns the demo client.
...
export const HierarchyProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  /**
   * Tracks the previous user identity to detect account switches.
   * When user.id or user.qualification changes, we fully reset hierarchy state
   * to prevent data from the previous session leaking into the new one.
   */
  const prevUserRef = useRef<{ id: string; qualification?: string } | null>(null);
  ...
  useEffect(() => {
    if (!user) return;
    // Visitors use fixed IDs — skip localStorage hydration
  ...
  useEffect(() => {
    ...
    if (!user) return;
    // Don't persist visitor fixed IDs — they are deterministic
  ...
  useEffect(() => {
    ...
    // Guard: do NOT initialize hierarchy until qualification is resolved.
    // This prevents entering the wrong branch with an incomplete user object.
    if (!user.qualification) return;

    // Detect identity or qualification change → full reset before re-init
    const prev = prevUserRef.current;
    if (prev && (prev.id !== user.id || prev.qualification !== user.qualification)) {
      dispatch({ type: 'RESET' });
      localStorage.removeItem(HIERARCHY_STORAGE_KEY);
      paginationRef.current = {
        analystsOffset: 0,
    ...
    prevUserRef.current = { id: user.id, qualification: user.qualification };

    // Visitor: treat like a non-admin user bound to the fixed visitor analyst.
    // Real DB records exist (migration 024 + 025), so the normal flow works.
    if (user.qualification === 'visitante') {
      dispatch({
        type: 'SET_SELECTED_ANALYST',
        payload: {
      ...
      // loadClients will fire via the effectiveAnalystId effect
      // and fetch the real demo client from the DB.
      return;
    }
