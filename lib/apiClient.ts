 ...
 */
export async function apiClient<T = unknown>(url: string, options: ApiRequestOptions = {}): Promise<T> {
  const {
    timeout = 30000,
    retries = 3,
  ...
  return withApiRetry(async () => {
    ...
    try {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        logger.error(`${operationName}: Timeout after ${timeout}ms`, error, {
          component: 'ApiClient',
          timeout,
          duration: `${duration.toFixed(2)}ms`,
        });
      ...
      // Outros erros
      logger.error(`${operationName}: Network error`, error instanceof Error ? error : new Error(String(error)), {
        component: 'ApiClient',
        duration: `${duration.toFixed(2)}ms`,
      });

      throw new ApiError(error instanceof Error ? error.message : 'Erro de rede', 0, 'NETWORK_ERROR');
    } finally {
 ...
 */
export const api = {
  get: <T = unknown>(url: string, options?: ApiRequestOptions) => apiClient<T>(url, { ...options, method: 'GET' }),
