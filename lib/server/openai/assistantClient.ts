...
function getOpenAIApiKey(): string {
  ...
  if (!apiKey || apiKey.trim() === '') {
    const errorMsg =
      'OPENAI_API_KEY não definida nas variáveis de ambiente do servidor. ' +
...
export async function callAssistant(question: string): Promise<AssistantResponse> {
  ...
  try {
    ...
    let finalModel = null;
    try {
      const finalRunRes = await fetch(`https://api.openai.com/v1/threads/${threadId}/runs/${runId}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${OPENAI_API_KEY}`,
          'Content-Type': 'application/json',
    ...
    const msgRes = await fetch(`https://api.openai.com/v1/threads/${threadId}/messages?limit=1&order=desc`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${OPENAI_API_KEY}`,
        'Content-Type': 'application/json',
