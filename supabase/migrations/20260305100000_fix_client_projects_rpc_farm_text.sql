-- Fix: farm IDs are TEXT (e.g. farm-{timestamp}-{random}), not UUID.
...
BEGIN
  ...
  WHERE
    ...
    AND (
      p_farm_id IS NULL OR p_farm_id = ''
      OR EXISTS (
        SELECT 1
...
GRANT EXECUTE ON FUNCTION public.client_list_projects_by_farm(UUID, TEXT) TO authenticated;

COMMENT ON FUNCTION public.client_list_projects_by_farm(UUID, TEXT)
IS 'Lista readonly de projetos para cliente autenticado por client_id e farm_id (TEXT), com fallback de legado.';
