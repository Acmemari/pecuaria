        ...
        <div className="hidden print:block min-h-screen bg-white font-sans text-slate-900 questionnaire-print-container">
            ...
            <div className="max-w-5xl mx-auto bg-white overflow-hidden shadow-none border-0 p-8 print:p-8">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-5 text-slate-400 text-sm">
                                <span className="flex items-center gap-1.5"><MapPin size={16} /> {questionnaire.farm_name || 'Fazenda não identificada'}</span>
                                <span className="flex items-center gap-1.5"><Calendar size={16} /> {diagnosisDate}</span>
                                <span className="flex items-center gap-1.5 text-emerald-400 font-semibold"><TrendingUp size={16} /> {results.totalQuestions} Pontos Analisados</span>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center justify-center bg-white/5 backdrop-blur-md border border-white/10 p-6 rounded-3xl min-w-[200px]">
                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Score Global</span>
                            <div className="flex items-baseline">
                                <span className={`text-6xl font-black leading-none ${results.finalScore >= 80 ? 'text-emerald-400' :
                                    results.finalScore >= 60 ? 'text-yellow-400' : 'text-rose-400'
                                    }`}>
                                    {results.finalScore}
                                </span>
                            ...
                            return (
                                <div key={groupName} className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
                                    ...
                                    <p className="text-xs text-slate-400 mt-3 font-medium">
                                        {g.categories.length} categorias analisadas
                                    </p>
                                </div>
                            );
                        })}
                                        ...
                                        return (
                                            <tr key={`${item.category}-${idx}`} className="hover:bg-slate-50/50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <span className={`text-[10px] font-bold px-2 py-1 rounded border ${COLORS.groups[item.group as keyof typeof COLORS.groups] ? `bg-opacity-10 text-opacity-100` : 'bg-gray-100 text-gray-600'}`} style={{
                ...
                <footer className="mt-12 p-8 border-t border-slate-100 flex flex-col sm:row justify-between items-center gap-4 text-slate-400 text-xs font-medium print:break-inside-avoid">
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 bg-slate-800 rounded flex items-center justify-center text-white font-bold text-[10px]">P</div>
